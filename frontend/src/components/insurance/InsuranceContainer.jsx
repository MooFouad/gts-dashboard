...
const InsuranceContainer = () => {
  ...
  useEffect(() => {
    console.log('Insurance tab loaded. Loading data from database...');
    loadFromDatabase();
  }, []);
  ...
  useEffect(() => {
    window.dispatchEvent(new CustomEvent('itemCountUpdate', {
      detail: { type: 'insurance', count: items.length }
    }));
  }, [items]);
  ...
  const loadFromDatabase = async () => {
    ...
    try {
      console.log('📁 Loading insurance records from database...');
      ...
      if (response.data.success) {
        ...
        console.log(`✅ Loaded ${dbRecords.length} insurance records from database`);
        setItems(dbRecords);
      }
    } catch (error) {
  ...
  const handleExport = () => {
    try {
      // Simple CSV export
      const headers = ['Plate Number', 'Insurance Company', 'Policy Number', 'Policy Status', 'Start Date', 'End Date', 'Insurance Type', 'Policy Holder'];
      const csvContent = [
        headers.join(','),
        ...items.map(item => [
          item.plateNumber || '',
          item.insuranceCompanyName || '',
          item.mainPolicyNumber || '',
          item.policyStatus || '',
      ...
      link.download = `insurance_records_${new Date().toISOString().split('T')[0]}.csv`;
      link.click();
    } catch (error) {
  ...
  const handleSyncFromAPI = async () => {
    if (!confirm('This will sync insurance data for ALL vehicles from Absher API. This may take several minutes. Continue?')) {
      return;
    }
    ...
    try {
      const token = localStorage.getItem('token');
      console.log('🔄 Starting bulk insurance sync...');

      const response = await axios.post(
        `${API_URL}/insurance/bulk-sync-from-api`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
        <h2 className="text-xl font-semibold">
          Vehicle Insurance ({items.length})
        </h2>
      ...
      {loading && items.length === 0 ? (
        <div className="text-center py-8 text-gray-500">
          <p className="mb-2">Loading insurance records...</p>
        </div>
      ) : items.length === 0 ? (
        <div className="text-center py-8 text-gray-500">
          <p className="mb-2">No insurance records found.</p>
          <p className="text-sm">Make sure you have synced Istemarah data first, then click "Sync All from API" to fetch insurance data for all vehicles.</p>
        </div>
      ) : (
        <>
